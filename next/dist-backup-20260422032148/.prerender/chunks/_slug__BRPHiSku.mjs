...
async function getStaticPaths() {
  ...
  return venues.filter((venue) => eatTypes.includes(venue.data.type)).map((venue) => ({
...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const related = allVenues.filter((entry) => routeSlug(entry) !== slug).filter((entry) => eatTypes.includes(entry.data.type)).filter((entry) => String(entry.data.place?.id ?? entry.data.place) === String(venue.data.place?.id ?? venue.data.place)).slice(0, 3);
  ...
  return renderTemplate`${renderComponent($$result, "BaseLayout", $$BaseLayout, { "title": `${venue.data.name}, ${venuePlaceName} · Peninsula Insider`, "description": description, "section": "eat", "canonical": canonical, "ogImage": ogImage }, { "default": async ($$result2) => renderTemplate(_a || (_a = __template([' <script type="application/ld+json">', "<\/script> ", " "])), unescapeHTML(JSON.stringify(venueSchema)), renderComponent($$result2, "VenueDetailTemplate", $$VenueDetailTemplate, { "venue": venue, "section": "eat", "related": related, "articles": articles, "itineraries": itineraries })) })}`;
}, "/home/node/.openclaw/workspace/peninsula-insider/next/src/pages/eat/[slug].astro", void 0);

const $$file = "/home/node/.openclaw/workspace/peninsula-insider/next/src/pages/eat/[slug].astro";
const $$url = "/eat/[slug]";
