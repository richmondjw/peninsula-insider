...
async function getStaticPaths() {
  ...
  return venues.filter((venue) => stayTypes.includes(venue.data.type)).map((venue) => ({
...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const related = allVenues.filter((entry) => routeSlug(entry) !== slug).filter((entry) => stayTypes.includes(entry.data.type)).slice(0, 3);
  ...
  return renderTemplate`${renderComponent($$result, "BaseLayout", $$BaseLayout, { "title": `${venue.data.name}, ${venuePlaceName} · Peninsula Insider`, "description": description, "section": "stay", "canonical": canonical, "ogImage": ogImage }, { "default": async ($$result2) => renderTemplate(_a || (_a = __template([' <script type="application/ld+json">', "<\/script> ", " "])), unescapeHTML(JSON.stringify(venueSchema)), renderComponent($$result2, "VenueDetailTemplate", $$VenueDetailTemplate, { "venue": venue, "section": "stay", "related": related, "articles": articles, "itineraries": itineraries })) })}`;
}, "/home/node/.openclaw/workspace/peninsula-insider/next/src/pages/stay/[slug].astro", void 0);

const $$file = "/home/node/.openclaw/workspace/peninsula-insider/next/src/pages/stay/[slug].astro";
const $$url = "/stay/[slug]";
