/**
 * Sanity → Astro adapters for Phase 5/6 entities:
 ...
 *
 * Phase 6 adds `fetchAll*FromSanity()` bulk variants that replace the
 * dual-read (getCollection + sanityReadEnabled fan-out) pattern, matching
 * the Phase 4/5 pattern used for venues, places, itineraries, and events.
 */
...
// ── All-experiences (Phase 6) ────────────────────────────────────────────────
const allExperiencesQuery = `*[_type == "experience" && !(_id in path("drafts.**"))] | order(name asc){
...
export async function fetchAllExperiencesFromSanity() {
  const docs: any[] = (await sanityClient.fetch(allExperiencesQuery)) ?? []
  return docs.map((d) => ({
    id: d.slug,
    slug: d.slug,
    data: {
...
// ── All-tours (Phase 6) ──────────────────────────────────────────────────────
const allToursQuery = `*[_type == "tour" && !(_id in path("drafts.**"))] | order(name asc){
...
export async function fetchAllToursFromSanity() {
  const docs: any[] = (await sanityClient.fetch(allToursQuery)) ?? []
  return docs.map((d) => ({
    id: d.slug,
    slug: d.slug,
    data: {
...
// ── All-tour-operators (Phase 6) ─────────────────────────────────────────────
const allTourOperatorsQuery = `*[_type == "tourOperator" && !(_id in path("drafts.**"))] | order(name asc){
...
export async function fetchAllTourOperatorsFromSanity() {
  const docs: any[] = (await sanityClient.fetch(allTourOperatorsQuery)) ?? []
  return docs.map((d) => ({
    id: d.slug,
    slug: d.slug,
    data: {
...
// ── All-tour-packages (Phase 6) ──────────────────────────────────────────────
const allTourPackagesQuery = `*[_type == "tourPackage" && !(_id in path("drafts.**"))] | order(name asc){
...
export async function fetchAllTourPackagesFromSanity() {
  const docs: any[] = (await sanityClient.fetch(allTourPackagesQuery)) ?? []
  return docs.map((d) => ({
    id: d.slug,
    slug: d.slug,
    data: {
...
// ── Experience (single-doc, kept for preview mode) ─────────────────────────
