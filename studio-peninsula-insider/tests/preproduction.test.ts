...
import { artifactDependenciesCurrent, evaluateArtifactGates, hashValue, resolveArtifactPath, withArtifactHash } from '../server/fixture-runner.js';
import {
...
async function assertStaleLineageRejected(
  ...
  const response = await request(app).put(`/api/foundry/runs/${run.id}/artifacts/${artifact.id}`).send({
  ...
  if (response.status === 400) {
    expect(response.body.error).toMatch(/lineage|Factual public field|resolve/i);
    return run;
  }
  expect(response.status).toBe(200);
  const edited = response.body as FoundryRun;
...
describe('governed pre-production recipes', () => {
  ...
  it('detects appended spoken, scene and overlay segments that do not have atomic lineage', () => {
    ...
    const scriptGates = evaluateArtifactGates(script.payload, podcast.claimSet.claims, script.factualSegmentIds, script.claimUsage, podcast.updatedAt, undefined, script.type);
    ...
    const sceneGates = evaluateArtifactGates(scenes.payload, video.claimSet.claims, scenes.factualSegmentIds, scenes.claimUsage, video.updatedAt, undefined, scenes.type);
    ...
    const overlayGates = evaluateArtifactGates(overlays.payload, video.claimSet.claims, overlays.factualSegmentIds, overlays.claimUsage, video.updatedAt, undefined, overlays.type);
  ...
  it('requires exact asset, placement, surface, rights and release bindings before media readiness can pass', () => {
    ...
    const parsed = ArtifactVersionSchema.parse(withArtifactHash(shots, run.claimSet.claims));
    ...
    podcastScript.payload.boundary.recognisablePeople = true;
