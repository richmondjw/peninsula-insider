...
import {
  ...
  NEWSLETTER_WEATHER_EVIDENCE_TEXT,
  runNewsletterSocialFixture,
} from '../server/newsletter-social-fixtures.js';
...
async function harness(evaluationClock?: () => string) {
  ...
  const store = new FileFoundryStore(file, directory, undefined, evaluationClock);
  return { app: createApp(store), store, file, directory };
}
...
describe('Insider Note and social recipes', () => {
  ...
  it('binds the weather locator and excerpt to exact sealed source bytes across restart and export', async () => {
    const base = runNewsletterSocialFixture('fixture-editor', 'weather-source-binding-v1');
    const weatherSource = base.bundle.sourceItems.find((source) => source.id === 'source-weather');
    const weatherClaim = base.claimSet.claims.find((claim) => claim.id === 'claim-weather');
    const weatherEvidence = weatherClaim?.evidence[0];
    expect(weatherSource).toMatchObject({ kind: 'text_note', contentHash: hash(NEWSLETTER_WEATHER_EVIDENCE_TEXT) });
    expect(weatherEvidence).toMatchObject({
      sourceItemId: 'source-weather',
      locatorType: 'manual',
      locator: 'weather:1',
      excerpt: NEWSLETTER_WEATHER_EVIDENCE_TEXT,
      excerptHash: hash(NEWSLETTER_WEATHER_EVIDENCE_TEXT),
    });
    expect(weatherEvidence?.excerptHash).toBe(weatherSource?.contentHash);

    const { store, file, directory } = await harness();
    const created = await store.create(base);
    const restarted = new FileFoundryStore(file, directory);
    const reloaded = await restarted.get(created.id) as FoundryRun;
    const issue = artifact(reloaded, 'insider_note_issue');
    const reviewed = await restarted.review(reloaded.id, {
      artifactId: issue.id,
      decision: 'accepted',
      reviewer: 'fixture-reviewer',
      expectedVersion: reloaded.version,
      expectedArtifactVersion: issue.version,
    });
    const handoff = JSON.parse(buildArtifactHandoff(reviewed, issue.id)) as {
      publicFieldLineage: Array<{ segments: Array<{ source: string; claimId?: string; claimHash?: string }> }>;
    };
    const exportedWeatherSegments = handoff.publicFieldLineage
      .flatMap((field) => field.segments)
      .filter((segment) => segment.source === 'claim' && segment.claimId === 'claim-weather');
    expect(exportedWeatherSegments.length).toBeGreaterThan(0);
    expect(exportedWeatherSegments.every((segment) => segment.claimHash === hashValue(weatherClaim))).toBe(true);
  });
  ...
  it('rejects client-invented claim references and any newsletter image input', async () => {
    const { app } = await harness();
    const run = (await request(app).post('/api/foundry/runs').send({
      fixtureId: 'red-hill-newsletter-social', actor: 'fixture-editor', idempotencyKey: 'server-owned-newsletter-lineage-v1',
    }).expect(201)).body as FoundryRun;
    const issue = artifact(run, 'insider_note_issue');
    const forgedIssue = structuredClone(issue.payload);
    if (forgedIssue.positions[2].status !== 'included') throw new Error('Lead position omitted');
    forgedIssue.positions[2].body.claimIds = ['claim-does-not-exist'];
    await request(app).put(`/api/foundry/runs/${run.id}/artifacts/${issue.id}`).send({
      editor: 'lineage-adversary', expectedVersion: run.version, expectedArtifactVersion: issue.version, payload: forgedIssue,
    }).expect(400);

    const imagedIssue = structuredClone(issue.payload) as typeof issue.payload & { image?: unknown };
    imagedIssue.image = { assetId: 'client-image', placement: 'email-lead' };
    await request(app).put(`/api/foundry/runs/${run.id}/artifacts/${issue.id}`).send({
      editor: 'image-adversary', expectedVersion: run.version, expectedArtifactVersion: issue.version, payload: imagedIssue,
    }).expect(400);

    const carousel = artifact(run, 'instagram_carousel_script');
    const forgedCarousel = structuredClone(carousel.payload);
    forgedCarousel.slides[0].claimIds = ['claim-restricted-client-invention'];
    await request(app).put(`/api/foundry/runs/${run.id}/artifacts/${carousel.id}`).send({
      editor: 'lineage-adversary', expectedVersion: run.version, expectedArtifactVersion: carousel.version, payload: forgedCarousel,
    }).expect(400);
  });
  ...
  it('reconciles expiring booking evidence when the injected evaluation clock advances', async () => {
    const addBookingExpiry = (source: FoundryRun) => {
      ...
      run.artifactPack.completed = run.artifactPack.completed.map((item) => ({
        ...
        dependencies: item.dependencies.map((dependency) => dependency.kind === 'claim_set'
          ...
          : dependency),
        publicFieldLineage: buildPublicFieldLineage(item.type, item.payload, item.claimUsage, run.claimSet.claims),
      })) as ArtifactVersion[];
      return run;
    };
    ...
    expect(JSON.parse(buildArtifactHandoff(reviewedEarly, earlyIssue.id))).toMatchObject({
  ...
  it('persists independent reviews and exports only the accepted artifact handoff', async () => {
    ...
    expect(handoff.body).toMatchObject({ authority: 'draft_handoff_only', publicationAuthority: false });
    expect(handoff.body.payload).toMatchObject({ sendAuthority: 'james_only', scheduleAt: null });
    ...
    expect(buildArtifactHandoff(run, artifact(run, 'insider_note_subject_set').id)).toContain('"selectedPairId": null');
  });
