...
import { PreproductionPayloadSchema, type PreproductionPayload } from '../shared/preproduction-contracts.js';
...
function timingCurrent(payload: PreproductionPayload): boolean {
  switch (payload.kind) {
    ...
    case 'podcast_run_sheet': return orderedAndBounded(payload.segments, payload.targetDurationMs, true);
    case 'podcast_chapters': return orderedAndBounded(payload.items, payload.targetDurationMs, false);
    case 'video_script': return payload.segments.reduce((total, segment) => total + segment.durationMs, 0) === payload.targetSeconds * 1_000;
    case 'video_shot_list': return payload.shots.reduce((total, shot) => total + shot.durationMs, 0) === payload.targetDurationMs;
    case 'video_scenes': return orderedAndBounded(payload.scenes, payload.targetDurationMs, true);
    case 'video_overlays': return orderedAndBounded(payload.items, payload.targetDurationMs, false);
    case 'video_subtitles': return orderedAndBounded(payload.cues, payload.targetDurationMs, false);
    default: return true;
  }
}
...
function policyCurrent(payload: PreproductionPayload): boolean {
  ...
  if (JSON.stringify(payload).includes('"contentMode":"simulated_contributor"')) return false;
...
function requiredMediaPlacements(payload: PreproductionPayload): Array<{ path: string; surface: MediaAssignment['surface'] }> {
  switch (payload.kind) {
    case 'explainer_carousel': return payload.slides.map((_, index) => ({ path: `$.slides[${index}]`, surface: payload.boundary.generationMode === 'illustrative' ? 'illustration' : 'image' }));
    case 'explainer_visualisation': return payload.points.map((_, index) => ({ path: `$.points[${index}]`, surface: 'illustration' }));
    case 'explainer_voiceover':
    case 'podcast_script':
    case 'video_script': return payload.segments.map((_, index) => ({ path: `$.segments[${index}]`, surface: 'audio' }));
    case 'video_hooks': return payload.variants.map((_, index) => ({ path: `$.variants[${index}]`, surface: 'audio' }));
    case 'video_shot_list': return payload.shots.map((shot, index) => ({ path: `$.shots[${index}]`, surface: shot.source === 'approved_illustrative' ? 'illustration' : 'video' }));
    case 'video_scenes': return payload.scenes.flatMap((scene, index) => scene.treatment === 'text_only' ? [] : [{ path: `$.scenes[${index}]`, surface: scene.treatment === 'illustrative' ? 'illustration' as const : 'video' as const }]);
    case 'video_thumbnail': return payload.variants.map((_, index) => ({ path: `$.variants[${index}]`, surface: payload.boundary.generationMode === 'illustrative' ? 'illustration' : 'image' }));
    default: return [];
  }
}
...
export function mediaRightsBindingHash(assignment: MediaAssignment): string {
  return createHash('sha256').update(JSON.stringify({
    asset: assignment.asset, placementPath: assignment.placementPath, surface: assignment.surface,
    rights: assignment.rights, recognisablePeople: assignment.recognisablePeople, releaseIds: [...assignment.releaseIds].sort(),
  })).digest('hex');
}
...
export function mediaReadinessCurrent(payload: PreproductionPayload, dependencies: ArtifactDependency[]): boolean {
  ...
  if (new Set(assignmentRightsRefs).size !== assignmentRightsRefs.length || new Set(snapshotRightsRefs).size !== snapshotRightsRefs.length || new Set(dependencyRightsRefs).size !== dependencyRightsRefs.length) return false;
  if (!requiredPlacements.every((required) => boundary.mediaAssignments.some((assignment) => assignment.placementPath === required.path && assignment.surface === required.surface))) return false;
  ...
  if (['explainer_voiceover', 'podcast_script', 'video_hooks', 'video_script'].includes(payload.kind) && (boundary.voiceMode !== 'human_cleared' || !boundary.voiceReleaseId)) return false;
  ...
  return boundary.mediaAssignments.every((assignment) => {
    ...
    return snapshot?.contentHash === bindingHash && dependency?.kind === 'media_rights' && dependency.contentHash === bindingHash && dependency.status === 'cleared';
  });
}
...
export function evaluatePreproductionGates(payloadInput: unknown, dependencies: ArtifactDependency[]): GateResult[] {
  ...
  return [
    gate('preproduction_policy', policyPassed, policyPassed ? 'No simulated contributor, cloned voice, synthetic voice or generated documentary treatment is authorised.' : 'Simulated contributors, cloned or synthetic voice, generated documentary treatment, or undisclosed illustrative generation are blocked.'),
    gate('timing_valid', timingPassed, timingPassed ? 'Timed segments are ordered and match the declared duration.' : 'Timed segments overlap, leave an invalid boundary or do not match the declared duration.'),
    gate('media_render_ready', mediaPassed && payload.boundary.mediaStatus === 'ready', mediaPassed && payload.boundary.mediaStatus === 'ready' ? 'Every assigned medium has matching rights, releases and required disclosure.' : payload.boundary.mediaStatus === 'unassigned' && mediaPassed ? 'Text pre-production is reviewable; media and render readiness remain deliberately unassigned.' : 'Media or render readiness lacks exact rights, releases, approval or disclosure.', payload.boundary.mediaStatus === 'ready'),
  ];
}
