...
export const PreproductionArtifactTypeSchema = z.enum([
  ...
  'podcast_evidence_dossier', 'podcast_angle', 'podcast_run_sheet', 'podcast_interview_guide', 'podcast_script',
  'podcast_show_notes', 'podcast_chapters', 'video_hooks', 'video_script', 'video_shot_list', 'video_scenes',
  'video_overlays', 'video_subtitles', 'video_thumbnail', 'video_platform_captions',
]);

export const RightsSnapshotSchema = z.object({
  id: z.string().min(1), version: z.number().int().positive(), contentHash: Sha256Schema,
});
...
export const MediaAssignmentSchema = z.object({
  ...
  releaseIds: z.array(z.string().min(1)).default([]).refine(
    (values) => values.every((value, index) => index === 0 || value > values[index - 1]),
    'Release IDs must be unique and sorted.',
  ),
}).superRefine((assignment, context) => {
  if (!assignment.recognisablePeople && assignment.releaseIds.length !== 0) {
    context.addIssue({ code: 'custom', path: ['releaseIds'], message: 'No recognisable people requires no release IDs.' });
  }
  if (assignment.recognisablePeople && assignment.releaseIds.length === 0) {
    context.addIssue({ code: 'custom', path: ['releaseIds'], message: 'Recognisable people require exact release IDs.' });
  }
});
...
export const ProductionBoundarySchema = z.object({
  ...
  if (!boundary.recognisablePeople && boundary.releaseIds.length !== 0) {
    context.addIssue({ code: 'custom', path: ['releaseIds'], message: 'No recognisable people requires no release IDs.' });
  }
  if (boundary.recognisablePeople && boundary.releaseIds.length === 0) {
    context.addIssue({ code: 'custom', path: ['releaseIds'], message: 'Recognisable people require exact release IDs.' });
...
const TextSegmentSchema = z.object({ id: z.string().min(1), text: z.string().min(1) });
...
const TimedSegmentSchema = z.object({
  id: z.string().min(1), title: z.string().min(1), text: z.string().min(1),
...
export const VideoPlatformCaptionsPayloadSchema = z.object({
  kind: z.literal('video_platform_captions'), title: z.string().min(1), items: z.array(z.object({ id: z.string().min(1), platform: z.enum(['instagram_reel', 'facebook_reel', 'linkedin_video']), caption: z.string().min(1) })).min(1), ...Boundary,
});

export const PreproductionPayloadSchema = z.discriminatedUnion('kind', [
  ExplainerCorePayloadSchema, ExplainerFaqPayloadSchema, ExplainerCarouselPayloadSchema, ExplainerVoiceoverPayloadSchema,
  ExplainerVisualisationPayloadSchema, PodcastEvidenceDossierPayloadSchema, PodcastAnglePayloadSchema, PodcastRunSheetPayloadSchema,
  PodcastInterviewGuidePayloadSchema, PodcastScriptPayloadSchema, PodcastShowNotesPayloadSchema, PodcastChaptersPayloadSchema,
  VideoHooksPayloadSchema, VideoScriptPayloadSchema, VideoShotListPayloadSchema, VideoScenesPayloadSchema, VideoOverlaysPayloadSchema,
  VideoSubtitlesPayloadSchema, VideoThumbnailPayloadSchema, VideoPlatformCaptionsPayloadSchema,
]);
...
export function requiredPreproductionLineagePaths(payload: PreproductionPayload): string[] {
  const paths = (() => {
    switch (payload.kind) {
    case 'explainer_core': return ['$.thesis', ...payload.sections.map((_, index) => `$.sections[${index}].text`)];
    case 'explainer_faq': return payload.items.flatMap((_, index) => [`$.items[${index}].question`, `$.items[${index}].answer`]);
    case 'explainer_carousel': return payload.slides.flatMap((_, index) => [`$.slides[${index}].headline`, `$.slides[${index}].body`]);
    case 'explainer_voiceover':
    case 'podcast_script':
    case 'video_script': return payload.segments.flatMap((_, index) => [`$.segments[${index}].text`, `$.segments[${index}].speaker`]);
    case 'explainer_visualisation': return [...payload.points.flatMap((_, index) => [`$.points[${index}].label`, `$.points[${index}].valueText`]), '$.sourceNote'];
    case 'podcast_evidence_dossier': return payload.evidenceItems.map((_, index) => `$.evidenceItems[${index}].text`);
    case 'podcast_angle': return ['$.promise', '$.audience', ...payload.avoidClaims.map((_, index) => `$.avoidClaims[${index}]`)];
    case 'podcast_run_sheet': return payload.segments.flatMap((_, index) => [`$.segments[${index}].title`, `$.segments[${index}].text`]);
    case 'podcast_interview_guide': return [...payload.questions.flatMap((_, index) => [`$.questions[${index}].prompt`, `$.questions[${index}].rationale`]), ...payload.prohibitedPrompts.map((_, index) => `$.prohibitedPrompts[${index}]`)];
    case 'podcast_show_notes': return ['$.summary', ...payload.bullets.map((_, index) => `$.bullets[${index}].text`)];
    case 'podcast_chapters': return payload.items.flatMap((_, index) => [`$.items[${index}].title`, `$.items[${index}].text`]);
    case 'video_hooks': return payload.variants.flatMap((_, index) => [`$.variants[${index}].text`, `$.variants[${index}].speaker`]);
    case 'video_shot_list': return payload.shots.map((_, index) => `$.shots[${index}].description`);
    case 'video_scenes': return payload.scenes.flatMap((_, index) => [`$.scenes[${index}].title`, `$.scenes[${index}].text`]);
    case 'video_overlays': return payload.items.flatMap((_, index) => [`$.items[${index}].title`, `$.items[${index}].text`]);
    case 'video_subtitles': return payload.cues.flatMap((_, index) => [`$.cues[${index}].title`, `$.cues[${index}].text`]);
    case 'video_thumbnail': return payload.variants.flatMap((_, index) => [`$.variants[${index}].headline`, `$.variants[${index}].visualBrief`]);
    case 'video_platform_captions': return payload.items.map((_, index) => `$.items[${index}].caption`);
    }
  })();
  return ['$.title', ...paths];
}
